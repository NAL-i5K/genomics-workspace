...
blast_info = {
    'col_types': [
        'str', 'str', 'float', 'int', 'int', 'int', 'int', 'int', 'int', 'int',
...
def create(request, iframe=False):
    if request.method == 'GET':
        blastdb_list = sorted(
            [[
                db.type.dataset_type,
                db.type.get_molecule_type_display(), db.title,
                db.organism.display_name, db.description
            ] for db in BlastDb.objects.select_related('organism')
             .select_related('type').filter(is_shown=True) if db.db_ready()],
            key=lambda x: (x[3], x[1], x[0], x[2]))
        blastdb_type_counts = dict([(k.lower().replace(' ', '_'), len(
            list(g))) for k, g in groupby(
                sorted(blastdb_list, key=lambda x: x[0]), key=lambda x: x[0])])
        return render(
            request, 'blast/main.html', {
                'title': 'BLAST Query',
        ...
        # setup file paths
        task_id = uuid4(
        ).hex  # TODO: Create from hash of input to check for duplicate inputs
        file_prefix = path.join(settings.MEDIA_ROOT, 'blast', 'task', task_id,
                                task_id)
        ...
        if (path.getsize(query_filename) >
                int(settings.BLAST_QUERY_SIZE_MAX) * 1024):
            return render(
                request, 'blast/invalid_query.html', {
                    'title':
                    'Your query size is ' + str(path.getsize(query_filename)) +
                    ' bytes, but exceeds our query size limit of ' + str(
                        settings.BLAST_QUERY_SIZE_MAX) +
                    ' kbytes,  Please try again with a smaller query size.',
                })
        ...
        # build blast command
        db_list = ' '.join([
            db.fasta_file.path_full for db in BlastDb.objects.filter(
                title__in=set(request.POST.getlist('db-name')))
            if db.db_ready()
        ])
        if not db_list:
            return render(request, 'blast/invalid_query.html', {
        ...
        if request.POST['program'] in [
            ...
            with open(query_filename, 'r') as f:
                ...
                if (qstr.count('>') > int(settings.BLAST_QUERY_MAX)):
                    ...
                    return render(
                        request, 'blast/invalid_query.html', {
                            'title':
                            'Your search includes ' + query_cnt +
                            ' sequences, but blast allows a maximum of ' + str(
                                settings.BLAST_QUERY_MAX) +
                            ' sequences per submission.',
                        })

            # generate customized_options
            input_opt = []
            max_target_seqs = request.POST.get('max_target_seqs', 50)
            for blast_option in blast_customized_options[request.POST[
                    'program']]:
                if blast_option == 'low_complexity':
                    if request.POST['program'] == 'blastn':
                        input_opt.extend(
                            ['-dust', request.POST['low_complexity']])
                    else:
                        input_opt.extend(
                            ['-seg', request.POST['low_complexity']])
                else:
                    input_opt.extend(
                        ['-' + blast_option, request.POST[blast_option]])

            program_path = path.join(settings.BASE_DIR, 'blast', bin_name,
                                     request.POST['program'])
            num_threads = '4' if cpu_count() >= 4 else str(cpu_count())
            args_list = [[
                program_path, '-query', query_filename, '-db', db_list,
                '-outfmt', '11', '-out', asn_filename, '-num_threads',
                num_threads
            ]]
            args_list[0].extend(input_opt)

            # convert to multiple formats
            blast_formatter_path = path.join(settings.BASE_DIR, 'blast',
                                             bin_name, 'blast_formatter')
            for ext, outfmt in blast_info['ext'].items():
                args = [
                    blast_formatter_path, '-archive', asn_filename, '-outfmt',
                    outfmt, '-out', file_prefix + ext
                ]
                if ext == '.html':
                    args.append('-html')
            ...
            with open(query_filename, 'r') as f:
                ...
                if (seq_count == 0):
                    seq_count = 1
                with open(
                        path.join(path.dirname(file_prefix), 'status.json'),
                        'wb') as f:
...
def retrieve(request, task_id='1'):
    try:
        ...
        if r.result_date and (r.result_date.replace(tzinfo=None) >=
                              (datetime.utcnow() + timedelta(days=-7))):
            if r.result_status in ['SUCCESS', 'NO_GFF']:
                file_prefix = path.join(settings.MEDIA_ROOT, 'blast', 'task',
                                        task_id, task_id)
                results_info = ''
                with open(
                        path.join(settings.MEDIA_ROOT, 'blast', 'task',
                                  task_id, 'info.json'), 'rb') as f:
                    results_info = f.read()
                results_data = ''
                ...
                results_col_names_display = ['BLAST Database Title'
                                             ] + results_col_names_display
                return render(
                    request, 'blast/results.html', {
                        'title':
                        'BLAST Result',
                        'results_col_names':
                        json.dumps(results_col_names),
                        'results_col_names_display':
                        json.dumps(results_col_names_display),
                        'results_detail':
                        json.dumps(results_detail),
                        'results_data':
                        results_data,
                        'results_info':
                        results_info,
                ...
                return render(
                    request, 'blast/results_not_existed.html', {
                        'title': 'No Hits Found',
                        'isNoHits': True,
                        'isExpired': False,
                    })
        else:
            enqueue_date = r.enqueue_date.astimezone(
                timezone('US/Eastern')).strftime('%d %b %Y %X %Z')
            if r.dequeue_date:
                dequeue_date = r.dequeue_date.astimezone(
                    timezone('US/Eastern')).strftime('%d %b %Y %X %Z')
            else:
                dequeue_date = None
            # result is exipired
...
def read_gff3(request, task_id, dbname):
    ...
    try:
        if request.method == 'GET':
            with open(
                    path.join(settings.MEDIA_ROOT, 'blast', 'task', task_id,
                              dbname) + '.gff', 'rb') as f:
                output = f.read()
    finally:
...
def status(request, task_id):
    if request.method == 'GET':
        status_file_path = path.join(settings.MEDIA_ROOT, 'blast', 'task',
                                     task_id, 'status.json')
        ...
        if path.isfile(status_file_path):
            with open(status_file_path, 'rb') as f:
                ...
                if statusdata['status'] == 'pending' and settings.USE_CACHE:
                    ...
                    asn_path = path.join(settings.MEDIA_ROOT, 'blast', 'task',
                                         task_id, (task_id + '.asn'))
