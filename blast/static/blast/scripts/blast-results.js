...
$(function() {
  ...
  // Prepare Data //
  //////////////////
  // result_col_names is from template
  ...
  var State = Backbone.Model.extend({
    ...
    focus_row_index: function() {
      ...
      if (focus_row_index == null) {
        ...
        if (selected.length > 0) focus_row_index = selected[0];
        else focus_row_index = last_focus_row_index;
      }
  ...
  cm_blast_report.on("cursorActivity", function(instance) {
    ...
    var filtered = results_info["line_num_list"].filter(function(i) {
      return i <= cm_blast_report.getCursor().line + 3;
    });
    var i = 0;
    if (filtered.length > 0) i = filtered.length - 1;
  ...
  function cm_blast_report_select_hsp(row_idx) {
    cm_blast_report.operation(function() {
      cm_blast_report.scrollIntoView(
        { line: results_info["line_num_list"][row_idx], ch: 0 },
        cm_blast_report.getScrollInfo().clientHeight / 2
      );
      cm_blast_report.setCursor({
        line: results_info["line_num_list"][row_idx],
        ch: 0
      });
  ...
  s.on(
    ...
    function(model, value, options) {
      if (options.set_by == this) return;
      if (value != null) cm_blast_report_select_hsp(value);
      else {
        selected = s.get("selected");
        if (selected.length > 0) cm_blast_report_select_hsp(selected[0]);
      }
    },
    cm_blast_report
  );
  s.on(
    ...
    function(model, value, options) {
      if (options.set_by == this) return;
      if (value.length > 0) cm_blast_report_select_hsp(value[0]);
    },
    cm_blast_report
  );
  ...
  // FASTA Viewer //
  //////////////////
  var empty_fasta_hint =
    "Select one or more rows in the table on the left to display its FASTA sequence.";
  ...
  function get_fasta(sseqid) {
    ...
    if (sseqid in fasta_cache) return true;
    else if (sseqid in fasta_loading) return fasta_loading[sseqid];
    else {
      fasta_loading[sseqid] = $.get(
        url_root + "/api/seq/" + sseqid + "/?format=fasta",
        function(data) {
  ...
  s.on(
    "change:selected",
    function(model, row_indexes, options) {
      if (options.set_by == this) return;
      if (row_indexes.length > 0) {
        sseqids = _.uniq(
          row_indexes.map(function(row_index) {
            return results_data[row_index][col_idx["sseqid"]];
          })
        );
        $.when.apply(null, sseqids.map(get_fasta)).then(function() {
          ...
          if (
            sseqids.length > 0 &&
            _.all(
              sseqids.map(function(sseqid) {
                return sseqid in fasta_cache;
              })
            )
          ) {
            var fasta = "";
            for (var i = 0; i < sseqids.length; i++) {
  ...
  var $results_table = $("#results-table").dataTable({
    ...
    dom:
      '<"' +
      toolbar_prefix +
      'tl ui-corner-tr"Rifr>' +
      "t" +
      '<"' +
      toolbar_prefix +
      'bl ui-corner-br"<"btn-download btn-group dropup">T>S',
    tableTools: {
      sRowSelect: "os",
      aButtons: []
    },
            ...
            if (dbtitle in results_info["db_url"]) {
              ...
              final_url = encodeURI(
                db_url +
                  ...
                  ".." +
                  end_pos +
                  '&addStores={"url":{"type":"JBrowse/Store/SeqFeature/GFF3","urlTemplate":"' +
                  /^(https?:\/\/)/g.exec(results_info["db_url"][dbtitle])[1] +
                  /https?:\/\/(.*?)\/(?:blast)+/g.exec(document.URL)[1] +
                  "/media/blast/task/" +
                  task_id +
                  "/" +
                  dbtitle +
                  '.gff"}}&addTracks=[{"label":"BLAST+ Results","category":"0. Reference Assembly","type":"WebApollo/View/Track/DraggableBLASTFeatures","store":"url","style":{"renderClassName":"gray-center-30pct","subfeatureClasses":{"match_part":"blast-match_part"}}}]&tracks=BLAST+ Results'
              );

              return (
                '<a class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="right" data-container="body" title="' +
                dbtitle +
                '\nClick to view in genome browser" target="_blank" href="' +
                final_url +
                '" role="button"><span class="glyphicon glyphicon-new-window"></span> ' +
                results_info["db_organism"][dbtitle] +
                "</a>"
              );
            } else {
              return (
    ...
    headerCallback: function(thead, data, start, end, display) {
      $(thead)
        ...
        .each(function(index) {
          $(this)
            .children()
            .tooltip("destroy"); // remove old tooltip
          $(this).html(
            '<a data-toggle="tooltip" data-placement="top" data-container="body" title="' +
              results_col_names_display[col_idx[$(this).text()]] +
              '"><span>' +
              results_col_names[col_idx[$(this).text()]] +
              "</span></a>"
          );
          $(this)
            .children()
    ...
    rowCallback: function(row, data) {
      var $blastdb_td = $("td", row).eq(index_of_blastdb);
...
<ul class="dropdown-menu" role="menu">\
    <li><a href="' +
      task_path +
      '.0"><span class="glyphicon glyphicon-file"></span> Pairwise</a></li>\
    <li><a href="' +
      task_path +
      '.1"><span class="glyphicon glyphicon-file"></span> Query-anchored showing identities</a></li>\
    <li><a href="' +
      task_path +
      '.3"><span class="glyphicon glyphicon-file"></span> Flat query-anchored, show identities</a></li>\
    <li><a href="' +
      task_path +
      '.xml"><span class="glyphicon glyphicon-file"></span> XML</a></li>\
    <li><a href="' +
      task_path +
      '.tsv"><span class="glyphicon glyphicon-file"></span> Tabular</a></li>\
    <li><a href="' +
      task_path +
      '.csv"><span class="glyphicon glyphicon-file"></span> CSV</a></li>\
    <li><a href="' +
      task_path +
      '.asn"><span class="glyphicon glyphicon-file"></span> BLAST archive format (ASN.1)</a></li>\
</ul>'
  );
  ///////////////////
  // Table Filters //
  ...
  $(".dataTables_scrollFoot tfoot th").each(function(i) {
    ...
    if (type == "choice") {
      ...
      var select = $(
        '<select id="' +
          id +
          '" class="selectpicker dropup show-menu-arrow" data-style="btn-sm btn-default" data-width="91px" multiple data-live-search="true" data-actions-box="true" multiple data-selected-text-format="count" data-count-selected-text="{0} of {1}" title="Filter" data-icon="icon-filter" data-size="10"></select>'
      )
        .appendTo($(this).empty())
        .on("change", function() {
          ...
          var selected_options = $("#" + id + " option:selected")
            .map(function() {
              return this.value;
            })
            .get();
          ...
          if (selected_options.length == db_list.length) {
            ...
            results_table_api
              .column(i)
              .search(selected_options.join("|") || "^$", true, false)
              .draw();
          }
        });
      ...
      var input = $(
        '<div class="btn-group dropup show-menu-arrow">\
<button id="' +
          id +
          '-toggle" type="button" class="btn btn-sm btn-default dropdown-toggle" data-toggle="dropdown">\
...
<label for="' +
          id +
          '-input-max">Max</label></div>\
<div class="input-group">\
<input id="' +
          id +
          '-input-min" type="text" class="form-control col-search-input-min ' +
          title +
          '" placeholder="Min"/>\
<div class="input-group-addon">-</div>\
<input id="' +
          id +
          '-input-max" type="text" class="form-control col-search-input-max ' +
          title +
          '" placeholder="Max"/>\
</div><br>\
<div id="' +
          id +
          '-slider" class="slider"></div>\
</div></div>'
      ).appendTo($(this).empty());
      ...
      filters["min_" + i] = {
        ...
        filter: function(rd) {
          ...
          var val = rd[i] * 1;
          if (!_.isNaN(val) && (_.isNaN(i_min) || i_min <= val)) return true;
          else return false;
        }
      };
      filters["max_" + i] = {
        ...
        filter: function(rd) {
          ...
          var val = rd[i] * 1;
          if (!_.isNaN(val) && (_.isNaN(i_max) || i_max >= val)) return true;
          else return false;
        }
      };
      var draw_table = function() {
        if ($input_min.val() == min) filters["min_" + i]["enabled"] = false;
        else filters["min_" + i]["enabled"] = true;
        if ($input_max.val() == max) filters["max_" + i]["enabled"] = false;
        else filters["max_" + i]["enabled"] = true;
      ...
      $slider.slider({
        ...
        slide: function(event, ui) {
          // check for 0
          if (ui.values[0] == min_log) $input_min.val(0);
          else
            $input_min.val(
              Math.round10(Math.pow(10, ui.values[0]), ui.values[0])
            );
          $input_max.val(
            Math.round10(Math.pow(10, ui.values[1]), ui.values[1])
          );
        }
      });
      $input_min.val(min);
      ...
      $input_min.on("change", function() {
        ...
        if (!_.isNaN(i_min) && $input_min.val() != "") {
          if (i_min > i_max) i_min = i_max;
          else if (i_min < min) i_min = min;
          $input_min.val(i_min);
          $slider.slider("values", 0, Math.log(i_min) / Math.log(10));
        } else {
          $input_min.val(min);
...
<input id="' +
          id +
          '-input-min" type="number" min="' +
          min +
          '" max="' +
          max +
          '" class="form-control col-search-input-min ' +
          title +
          '" placeholder="Min" />\
<div class="input-group-addon">-</div>\
<input id="' +
          id +
          '-input-max" type="number" min="' +
          min +
          '" max="' +
          max +
          '" class="form-control col-search-input-max ' +
          title +
          '" placeholder="Max" />\
</div><br>\
<div id="' +
      ...
      var input = $(
        '<div id="' +
          id +
          '" class="input-group input-group-sm"><div class="input-group-addon"><span class="glyphicon glyphicon-search"></div><input type="text" class="form-control col-search-input ' +
          title +
          '" placeholder="' +
          title +
          ' filter" /></div></div>'
  ...
  $("#result-container").kendoSplitter({
    ...
    panes: [{ collapsible: true, size: "38%" }, { collapsible: false }]
  });
  $results_table.dataTableExt.afnFiltering.push(function(
    oSettings,
    aData,
    iDataIndex
  ) {
  ...
  function updateDataTableHeight() {
    ...
    var h =
      ...
      $ui_corner_tr.outerHeight() -
      $ui_corner_br.outerHeight() -
      $dataTables_scrollHead.outerHeight() -
      $dataTables_scrollFoot.outerHeight();
  ...
  s.on(
    "change:hover",
    function(model, hover_index, options) {
  ...
  s.on(
    "change:selected",
    function(model, selected_indexes, options) {
      if (options.set_by == this) return;
  ...
  // Alignment Graph //
  /////////////////////
  var sorted_score_data = results_table_api
    .column(col_idx["bitscore"])
    .data()
    .sort(num_asc);
  var dynamic_min_score = Math.round(
    sorted_score_data[Math.round((sorted_score_data.length - 1) * 0.1)] - 1
  ); // if there is only one score value
  var dynamic_max_score = Math.round(
    sorted_score_data[Math.round((sorted_score_data.length - 1) * 0.9)] + 1
  );
  var light_color_list = [
    "#fc9272",
    "#bcbddc",
    "#a1d99b",
    "#9ecae1",
  ...
  var light_color_interpolator = chroma
    .scale(light_color_list)
    .domain([min_score, max_score], 50);
  // #7f0000, #4d004b, #004529, #081d58, #000000
  var dark_color_interpolator = chroma
    .scale(dark_color_list)
    .domain([min_score, max_score], 50);
  ...
  function draw_bilinear(
    ...
    var top_color_interpolator = chroma
      .scale(top_color_list)
      .domain([legend_dim.left, legend_dim.width - legend_dim.right]);
    var bottom_color_interpolator = chroma
      .scale(bottom_color_list)
      .domain([legend_dim.left, legend_dim.width - legend_dim.right]);
    if (offsetX) {
      ...
      for (var x = 0; x < legend_dim.width; x++) {
        ...
        var y_color_list = [
          top_color_interpolator(x).hex(),
          bottom_color_interpolator(x).hex()
        ];
  ...
  function draw_legend_continuous(color_list, li_text) {
    ...
    // draw axis
    var range_max =
      legend_svg.attr("width") - legend_dim.left - legend_dim.right;
    var x = d3.scale
      .linear()
      .range([0, range_max])
    ...
    var xAxis = d3.svg
      ...
      .tickValues(
        d3.range(
          min_score,
          max_score + 1,
          (max_score - min_score) / (color_list.length - 1)
        )
      );
    ...
    legend_svg
      ...
      .attr(
        "transform",
        "translate(" + legend_dim.left + "," + legend_dim.top + ")"
      )
      .call(xAxis);
  }
  ...
  $("#score-color-canvas").mousemove(function(e) {
    ...
    var x = d3.scale
      ...
      .domain([legend_dim.left, legend_dim.width - legend_dim.left]);
  ...
  $("#score-to-color-checkbox").bootstrapSwitch({
    ...
    onInit: function(event, state) {
      $(".bootstrap-switch-id-score-to-color-checkbox").popover({
        ...
        content:
          "<b>Dynamic</b> color keys help visualize <i>relative</i> scores between all BLAST hits.<br><b>Static</b> color keys help visualize the <i>absolute</i> scores of each BLAST hit.",
        html: true,
        trigger: "hover",
        placement: "bottom"
      });
    ...
    onSwitchChange: function(event, state) {
      if (state) {
        ...
        // #fc9272, #bcbddc, #a1d99b, #9ecae1, #bdbdbd
        light_color_interpolator = chroma
          .scale(light_color_list)
          .domain([min_score, max_score], 50);
        // #7f0000, #4d004b, #004529, #081d58, #000000
        dark_color_interpolator = chroma
          .scale(dark_color_list)
          .domain([min_score, max_score], 50);
        // build color lookup tables
        ...
        color_scale = d3.scale
          .quantize()
          .domain([min_score, max_score])
          .range(_.zip(light_color_list, dark_color_list));
  ...
  function draw_legend(legend_id, color_list, li_text) {
    ...
    d3.select(legend_id)
      ...
      .style("background", function(c) {
        return (
          "-webkit-linear-gradient(top, " + c[0] + " 0%," + c[1] + " 100%)"
        );
  ...
  draw_legend(
    "#selected-hsp-color",
    [selected_color_gradient.slice(1)],
    function(c) {
      return "";
    }
  );
  draw_legend("#mouseover-hsp-color", [hover_color_gradient.slice(1)], function(
    c
  ) {
    return "";
  });
  ...
  $(document).keydown(function(event) {
    ...
    else if (event.which == "16") shift_down = true;
  });
  $(document).keyup(function(event) {
    if (event.which == "17") ctrl_down = false;
    else if (event.which == "16") shift_down = false;
  });
  ...
  function renderAlignmentGraph(canvas_name, focus_row_index) {
    ...
    // Get data indexes
    var rseqid =
      canvas_name == "query-canvas" ? col_idx["qseqid"] : col_idx["sseqid"];
    var rstart =
      canvas_name == "query-canvas" ? col_idx["qstart"] : col_idx["sstart"];
    var rend =
      canvas_name == "query-canvas" ? col_idx["qend"] : col_idx["send"];
    ...
    var center_position =
      focus_row_data[rend] < focus_row_data[rstart]
        ? focus_row_data[rend]
        : focus_row_data[rstart];
    var filtered_rows = results_table_api
      .rows({ search: "applied" })
      .eq(0)
      .filter(function(row_index) {
        ...
        var position =
          row_data[rend] < row_data[rstart] ? row_data[rend] : row_data[rstart];
        return (
          row_data[rseqid] == focus_row_data[rseqid] &&
          Math.abs(center_position - position) < hsp_nt_range
        );
      });
    // Draw at most 100 alignments, partition aligned_data if length > 100
    var graph_page_size = 50;
    var start =
      Math.floor(filtered_rows.indexOf(focus_row_index) / graph_page_size) *
      graph_page_size;
    var paged_filtered_rows = filtered_rows
      .toArray()
      .slice(start, start + graph_page_size);
    // Sort data ascending by coordinate for draw order
    //var sorted_data = _.sortBy(filtered_data, function (row) { return -row['bitscore']; });
    // Set name text
    $("#" + canvas_name + "-name").text(
      focus_row_data[rseqid] +
        ", BLAST Hits " +
        (start + 1) +
        "-" +
        (start + paged_filtered_rows.length)
    );
    ...
    _.each(paged_filtered_rows, function(row_index, paged_filtered_index) {
      ...
      var position =
        row_data[rend] < row_data[rstart] ? row_data[rend] : row_data[rstart];
      var length =
        row_data[rend] < row_data[rstart]
          ? row_data[rstart] - row_data[rend]
          : row_data[rend] - row_data[rstart];
      ...
      if (row_index == s.get("hover")) {
        feature = chart.addFeature(
          new BlockArrow("hover", position, length, strand)
        );
      } else if (_.indexOf(s.get("selected"), row_index) != -1) {
        feature = chart.addFeature(
          new BlockArrow("selected", position, length, strand)
        );
      } else {
        feature = chart.addFeature(
          new BlockArrow("hsp", position, length, strand)
        );
        // Color according to score
        var color_bitscore = Math.max(
          min_score,
          Math.min(max_score, Math.round(row_data[bitscore]))
        );
      ...
      feature.onClick = function() {
        if (ctrl_down) {
          ...
          if (_.indexOf(selected, row_index) != -1) {
            ...
            s.set(
              { selected: _.without(selected, row_index) },
              { set_by: "graph" }
            );
          } else {
    ...
    if ("hover" in chart) chart.hover.color = hover_color_gradient;
    ...
    if (!$canvas_clone.data("lane_size")) {
      optimal_lane_size =
        (canvas_clone.height - chart.getScaleHeight() - chart.trackBuffer) /
          chart.tracks[0].lanes.length -
        chart.laneBuffer;
      optimal_lane_size =
        optimal_lane_size < 5
          ? 5
          : optimal_lane_size > 20
            ? 20
            : optimal_lane_size;
  ...
  function updateAlignmentGraph() {
    document.getElementById("query-canvas").height =
      $query_canvas_container.outerHeight() - 25;
    document.getElementById("subject-canvas").height =
      $subject_canvas_container.outerHeight() - 25;
    document.getElementById(
      "query-canvas"
    ).width = $query_canvas_container.outerWidth();
    document.getElementById(
      "subject-canvas"
    ).width = $subject_canvas_container.outerWidth();
  ...
  $("#top-side-by-side-container").kendoSplitter({
    panes: [{ collapsible: false }, { collapsible: true, size: "50%" }]
  });
  $("#result-container")
    .data("kendoSplitter")
    .bind("resize", function() {
  ...
  $("#bottom-side-by-side-container").kendoSplitter({
    panes: [{ collapsible: false, size: w }, { collapsible: true }]
  });
  ...
  results_table_api.columns.adjust().draw();
  var footer = $(
    '<p class="nal-footer">2017 - National Agricultural Library</p>'
  );
