...
def create(request):
    ...
    if request.method == 'GET':
        ...
        if ("clustal_task_id" in request.GET):
            ...
            with io.open(clustal_aln, 'r') as content_file:
        ...
        with io.open(query_filename, 'r') as f:
            ...
            if (seq_count == 0):
                seq_count = 1
            with io.open(path.join(settings.MEDIA_ROOT, 'hmmer', 'task', task_id, 'status.json'), 'w') as f:
    ...
    try:
        ...
        if r.result_date and (r.result_date.replace(tzinfo=None) >= (datetime.utcnow() + timedelta(days=-7))):
            ...
            if path.isfile(dir_prefix + '.merge') and path.getsize(dir_prefix + '.merge') > 1024000:
                ...
                with io.open(dir_prefix + ".merge", 'r') as content_file:
                    for line in content_file:
                        line = line.rstrip('\n')
                        if line == '[ok]':
