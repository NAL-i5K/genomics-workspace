...
def create(request):
    ...
    if request.method == 'GET':
        hmmerdb_list = sorted(
            [[
                'Protein', "Protein", db.title, db.organism.display_name,
                db.description
            ] for db in HmmerDB.objects.select_related('organism').filter(
                is_shown=True)],
            key=lambda x: (x[3], x[1], x[0], x[2]))
        hmmerdb_type_counts = dict([(k.lower().replace(' ', '_'), len(
            list(g))) for k, g in groupby(
                sorted(hmmerdb_list, key=lambda x: x[0]), key=lambda x: x[0])])
        '''
        Redirect from clustal result
        '''
        clustal_content = []
        if ("clustal_task_id" in request.GET):
            clustal_aln = path.join(settings.MEDIA_ROOT, 'clustal', 'task',
        ...
        file_prefix = path.join(settings.MEDIA_ROOT, 'hmmer', 'task', task_id,
                                task_id)
        ...
        if 'query-file' in request.FILES:
            query_filename = path.join(settings.MEDIA_ROOT, 'hmmer', 'task',
                                       task_id,
        ...
        )  # Note that currently we didn't support HMMER on windows
        ...
        if request.POST['program'] == 'phmmer':
            with open(query_filename, 'r') as f:
                ...
                if qstr.count('>') > int(HMMER_QUERY_MAX):
                    ...
                    return render(
                        request, 'hmmer/invalid_query.html', {
                            'title':
                            'Your search includes ' + query_cnt +
                            ' sequences, but HMMER allows a maximum of ' +
                            str(HMMER_QUERY_MAX) +
            ...
            p = Popen(
                [
                    path.join(program_path, "hmmbuild"), "--fast", '--amino',
                    path.join(settings.MEDIA_ROOT, 'hmmer', 'task',
                              'hmmbuild.test'), query_filename
                ],
                stdout=PIPE,
                stderr=PIPE)
            p.wait()
            ...
            if (result != ''):
                return render(
                    request, 'hmmer/invalid_query.html', {
                        ...
                        'Invalid MSA format',
                        'info':
                        '<a href="http://toolkit.tuebingen.mpg.de/reformat/help_params#format" target="_blank"> \
        ...
        # build hmmer command
        db_list = [
            db.fasta_file.path_full for db in HmmerDB.objects.filter(
                title__in=set(request.POST.getlist('db-name')))
        ]
        for db in db_list:
            symlink(
                db,
                path.join(settings.MEDIA_ROOT, 'hmmer', 'task', task_id,
                          db[db.rindex('/') + 1:]))
        ...
        if request.POST['cutoff'] == 'evalue':
            option_params = [
                '--incE', request.POST['s_sequence'], '--incdomE',
                request.POST['s_hit'], '-E', request.POST['r_sequence'],
                '--domE', request.POST['r_hit']
            ]
        elif request.POST['cutoff'] == 'bitscore':
            option_params = [
                '--incT', request.POST['s_sequence'], '--incdomT',
                request.POST['s_hit'], '-T', request.POST['r_sequence'],
                '--domT', request.POST['r_hit']
            ]
        else:
            raise Http404
        ...
        args = generate_hmmer_args(request.POST['program'], program_path,
                                   query_filename, option_params, db_list)
...
def retrieve(request, task_id='1'):
    ...
    try:
        ...
        if r.result_date and (r.result_date.replace(tzinfo=None) >=
            ...
            url_base_prefix = path.join(settings.MEDIA_URL, 'hmmer', 'task',
                                        task_id)
            dir_base_prefix = path.join(settings.MEDIA_ROOT, 'hmmer', 'task',
                                        task_id)
            ...
            # 1mb limitation
            if path.isfile(dir_prefix + '.merge'
                           ) and path.getsize(dir_prefix + '.merge') > 1024000:
            ...
            if r.result_status == 'SUCCESS':
                ...
                return render(
                    request, 'hmmer/results_not_existed.html', {
                        'title': 'No Hits Found',
                        'isNoHits': True,
                        'isExpired': False,
                    })
...
def status(request, task_id):
    if request.method == 'GET':
        status_file_path = path.join(settings.MEDIA_ROOT, 'hmmer', 'task',
                                     task_id, 'status.json')
...
def generate_hmmer_args(program, program_path, query_filename, option_params,
                        db_list):
    args = []
    if program == 'hmmsearch':
        args.append([
            path.join(program_path, 'hmmbuild'), '--amino', '-o', 'hmm.sumary',
            query_filename + '.hmm', query_filename
        ])
        for idx, db in enumerate(db_list):
            args.append([
                path.join(program_path, 'hmmsearch'), '-o',
                str(idx) + '.out'
            ] + option_params + [query_filename + '.hmm', db])
    else:  # phmmer
        for idx, db in enumerate(db_list):
            args.append(
                [path.join(program_path, 'phmmer'), '-o',
                 str(idx) + '.out'] + option_params + [query_filename, db])
    return args
