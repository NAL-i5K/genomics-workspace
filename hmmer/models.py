...
class HmmerQueryRecord(models.Model):
    task_id = models.CharField(
        max_length=32,
        primary_key=True)  # ex. 128c8661c25d45b8-9ca7809a09619db9
    ...
    result_status = models.CharField(
        max_length=32, default='WAITING'
    )  # ex. WAITING, SUCCESS, NO_ASN, ASN_EMPTY, NO_CSV, CSV_EMPTY
...
class HmmerDB(models.Model):
    ...
    title = models.CharField(
        max_length=200, unique=True, default='')  # makeblastdb -title
    description = models.TextField(
        blank=True)  # shown in hmmer db selection ui
    is_shown = models.BooleanField(
        default=None,
        help_text='Display this database in the HMMER submit form'
    )  # to temporarily remove from hmmer db selection ui
