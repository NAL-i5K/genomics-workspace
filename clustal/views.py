        ...
        if request.POST['program'] in ['clustalw', 'clustalo']:
            ...
            if request.POST['program'] == 'clustalw':
                ...
                args_list_log.append([
                    'clustalw2', '-infile=' + path.basename(query_filename),
...
def retrieve(request, task_id='1'):
    ...
    try:
        ...
        if r.result_date and (r.result_date.replace(tzinfo=None) >=
            ...
            url_base_prefix = path.join(settings.MEDIA_URL, 'clustal', 'task',
                                        task_id)
            dir_base_prefix = path.join(settings.MEDIA_ROOT, 'clustal', 'task',
                                        task_id)
            ...
            if r.result_status in set(['SUCCESS']):
                ...
                return render(
                    request, 'clustal/results_not_existed.html', {
                        'title': 'No Hits Found',
                        'isNoHits': True,
                        'isExpired': False,
                    })
...
def status(request, task_id):
    ...
    '''
    if request.method == 'GET':
        status_file_path = path.join(settings.MEDIA_ROOT, 'clustal', 'task',
                                     task_id, 'status.json')
